...
tape( 'the function has an arity of 2', function test( t ) {
	t.strictEqual( mskmidrange.length, 2, 'returns expected value' );
	t.end();
});
...
tape( 'the function calculates the mid-range of an array (accessors)', function test( t ) {
	...
	x = [ 1.0, -2.0, -4.0, NaN, 5.0, 0.0, 3.0 ];
	mask = [ 0, 0, 0, 1, 0, 0, 0 ];
	v = mskmidrange( toAccessorArray( x ), toAccessorArray( mask ) );
	t.strictEqual( v, 0.5, 'returns expected value' );
